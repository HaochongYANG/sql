...
SELECT 'highest total sales' AS sales_label,
       market_date,
       total_sales
FROM ranked_sales
WHERE best_day_rank = 1
UNION
SELECT 'lowest total sales' AS sales_label,
       market_date,
       total_sales
FROM ranked_sales
...
SELECT vendor_name,
       product_name,
       SUM(5 * original_price) AS revenue_per_product
FROM vendor_customer_sales
